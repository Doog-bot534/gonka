...
func main() {
	...
	if subnetSigner != nil {
		...
		subnetEngine := internalsubnet.NewEngineAdapter(nodeBroker, configManager.GetCurrentNodeVersion(), payloadStore, chainPhaseTracker, httpClient)
		subnetValidator := internalsubnet.NewValidationAdapter(nodeBroker, configManager.GetCurrentNodeVersion(), chainPhaseTracker, httpClient, subnetBridge, recorder)
