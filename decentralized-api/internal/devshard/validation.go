...
func (v *ValidationAdapter) Validate(ctx context.Context, req devshard.ValidateRequest) (*devshard.ValidateResult, error) {
	...
	if validationUsage, err := validationResponse.GetUsage(); err == nil {
		if req.InputTokens > validationUsage.PromptTokens || req.OutputTokens > validationUsage.CompletionTokens {
			logging.Warn("devshard validation failed: inflated token counts",
...
func (v *ValidationAdapter) fetchPayloadsFromExecutor(ctx context.Context, req devshard.ValidateRequest, inferenceID string, epochID uint64) ([]byte, []byte, error) {
	...
	// Build request URL for devshard session endpoint
	requestURL, err := validation.BuildPayloadRequestURL(executorInfo.URL, fmt.Sprintf("v1/devshard/sessions/%s/payloads", req.EscrowID), inferenceID)
	if err != nil {
		return nil, nil, err
	}
	...
	// devshard.CanonicalizeJSON), so direct sha256 matches CanonicalPromptHash.
