...
- calling per-send receipt handlers when subnet_receipt arrives
...
10. `transport.HTTPClient`:
    ...
    - calls the per-send receipt handler when subnet_receipt arrives
...
- `Proxy` converts client requests into subnet inference requests (HTTP in/out).
