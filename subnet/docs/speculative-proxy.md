...
This note describes how `subnetctl` runs speculative inference in the subnet proxy.
...
This is the logic implemented in `subnet/cmd/subnetctl/speculative.go`.

## Why speculative execution exists

Subnet hosts can fail in several different ways:
...
Important detail: the proxy does not manually choose "host 2", "host 3", and so on. Instead, every call to `PrepareInference()` advances the session nonce, and the nonce naturally maps to the next host in the subnet group. That means repeated preparation automatically walks through the host set.
...
This makes the fallback logic universal across the whole subnet group, subject to the configured attempt limit.
...
This is important because every extra attempt is real subnet work and may later require timeout cleanup.
