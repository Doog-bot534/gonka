...
func setupDeadHostSession(t *testing.T, numHosts, aliveCount int, balance uint64, grace uint64) *Session {
	...
	for i := range clients {
		if i < aliveCount {
			sm := state.NewStateMachine("escrow-1", config, group, balance, userKey.Address(), verifier)
...
func TestFinalize_DoubleCall_AfterSuccess(t *testing.T) {
	...
	// Second finalize: must be idempotent (no error, no state change).
	err = session.Finalize(ctx)
	require.NoError(t, err)
	...
	// Settlement should still be buildable.
	sigs := session.Signatures()
...
func TestFinalize_DoubleCall_InsufficientQuorum(t *testing.T) {
	...
	// Send inferences (most will fail on dead hosts, that's expected).
	for i := 0; i < 5; i++ {
		session.SendInference(ctx, params) //nolint:errcheck
	}

	// First finalize: fails with insufficient signatures.
	err := session.Finalize(ctx)
	require.Error(t, err)
